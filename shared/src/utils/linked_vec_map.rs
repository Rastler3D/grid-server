...
/// A linked hash map.
pub struct LinkedVecMap<V> {
    map: VecMap<*mut Node<usize, V>>,
    head: *mut Node<usize, V>,
    free: *mut Node<usize, V>,
}
...
impl< V> LinkedVecMap<V> {
    /// Creates a linked hash map.
    pub fn new() -> Self {
        Self::with_map(VecMap::new())
    }

    /// Creates an empty linked hash map with the given initial capacity.
    pub fn with_capacity(capacity: usize) -> Self {
        Self::with_map(VecMap::with_capacity(capacity))
    }
}

impl<V> LinkedVecMap<V> {
    #[inline]
    fn detach(&mut self, node: *mut Node<usize,V>) {
        unsafe {
    ...
    #[inline]
    fn attach(&mut self, node: *mut Node<usize, V>) {
        unsafe {
    ...
    fn ensure_guard_node(&mut self) {
        if self.head.is_null() {
            ...
            unsafe {
                let node_layout = std::alloc::Layout::new::<Node<usize, V>>();
                self.head = std::alloc::alloc(node_layout) as *mut Node<usize, V>;
...
impl<V> LinkedVecMap<V> {
    fn with_map(map: VecMap<*mut Node<usize, V>>) -> Self {
        LinkedVecMap {
            map,
            head: ptr::null_mut(),
            free: ptr::null_mut(),
    ...
    pub fn reserve(&mut self, additional: usize) {
        self.map.reserve_len(additional);
    }
    ...
    /// ```
    pub fn entry(&mut self, k: usize) -> Entry<V> {
    ...
    /// ```
    pub fn entries(&mut self) -> Entries<V> {
    ...
    pub fn insert(&mut self, k: usize, v: V) -> Option<V> {
        ...
        let (node, old_val) = match self.map.get(k) {
            Some(node) => {
    ...
    /// ```
    pub fn get_refresh(&mut self, k: usize) -> Option<&mut V>
    {
        let (value, node_ptr_opt) = match self.map.get(k) {
            None => (None, None),
    ...
    /// ```
    #[inline]
    pub fn pop_front(&mut self) -> Option<(usize, V)> {
        if self.is_empty() {
            return None;
        }
    ...
    /// ```
    #[inline]
    pub fn front(&self) -> Option<(usize, &V)> {
        if self.is_empty() {
            return None;
        }
        let lru = unsafe { (*self.head).prev };
    ...
    /// ```
    #[inline]
    pub fn pop_back(&mut self) -> Option<(usize, V)> {
        if self.is_empty() {
            return None;
        }
    ...
    /// ```
    #[inline]
    pub fn back(&self) -> Option<(usize, &V)> {
        if self.is_empty() {
            return None;
        }
        let mru = unsafe { (*self.head).next };
    ...
    /// assert_eq!(&17, map.get(&"a").unwrap());
    /// ```
    pub fn iter_mut(&mut self) -> IterMut<usize, V> {
        let head = if self.head.is_null() {
            ptr::null_mut()
        } else {
            unsafe { (*self.head).prev }
    ...
    pub fn drain(&mut self) -> Drain<usize, V> {
        let len = self.len();
    ...
    /// ```
    pub fn values(&self) -> Values<usize, V> {
        Values { inner: self.iter() }
    }
}

impl<V> Index<usize> for LinkedVecMap<V>
{
    type Output = V;
...
impl<V> IndexMut<usize> for LinkedVecMap<V>
{
...
impl<V: Clone> Clone for LinkedVecMap<V> {
    fn clone(&self) -> Self {
...
impl<V> Default for LinkedVecMap<V> {
    fn default() -> Self {
        Self::new()
    }
}

impl<V> Extend<(usize, V)> for LinkedVecMap<V> {
...
impl<'a, V> Extend<(&'a usize, &'a V)> for LinkedVecMap<V>
    where
        V: 'a + Copy,
{
    fn extend<I: IntoIterator<Item = (&'a usize, &'a V)>>(&mut self, iter: I) {
        for (&k, &v) in iter {
            self.insert(k, v);
        }
    }
...
impl<V> iter::FromIterator<(usize, V)> for LinkedVecMap<V>
{
...
impl<V: fmt::Debug> fmt::Debug for LinkedVecMap<V>
{
...
impl<V: PartialEq> PartialEq for LinkedVecMap<V> {
    fn eq(&self, other: &Self) -> bool {
        self.len() == other.len() && self.iter().eq(other)
    }
}

impl<V: Eq> Eq for LinkedVecMap<V> {}

impl<V: PartialOrd> PartialOrd for LinkedVecMap<V>
{
...
impl<V: Ord> Ord for LinkedVecMap<V> {
    fn cmp(&self, other: &Self) -> Ordering {
        self.iter().cmp(other)
    }
}

impl<V: Hash> Hash for LinkedVecMap<V> {
...
unsafe impl<V: Send> Send for LinkedVecMap<V> {}

unsafe impl<V: Sync> Sync for LinkedVecMap<V> {}

impl<V> Drop for LinkedVecMap<V> {
    fn drop(&mut self) {
        if !self.head.is_null() {
            unsafe {
                self.drop_entries();
...
unsafe impl<'a, V> Send for Entries<'a, V>
    where
        V: Send,
{
}
...
unsafe impl<'a, V> Sync for Entries<'a, V>
    where
        V: Sync,
{
}
...
impl<'a, V> Iterator for Entries<'a, V> {
    type Item = OccupiedEntry<'a, V>;

    fn next(&mut self) -> Option<OccupiedEntry<'a, V>> {
        if self.remaining == 0 {
            None
        } else {
            self.remaining -= 1;
...
impl<'a, V> IntoIterator for &'a LinkedVecMap<V> {
    type Item = (&'a usize, &'a V);
    type IntoIter = Iter<'a, usize, V>;
    fn into_iter(self) -> Iter<'a, usize, V> {
        self.iter()
    }
}

impl<'a, V> IntoIterator for &'a mut LinkedVecMap<V> {
    type Item = (&'a usize, &'a mut V);
    type IntoIter = IterMut<'a, usize, V>;
    fn into_iter(self) -> IterMut<'a, usize, V> {
        self.iter_mut()
    }
}

impl<V> IntoIterator for LinkedVecMap<V> {
    type Item = (usize, V);
    type IntoIter = IntoIter<usize, V>;
    fn into_iter(mut self) -> IntoIter<usize, V> {
...
impl<'a, V> OccupiedEntry<'a, V> {
    ...
    pub fn insert(&mut self, value: V) -> V {
        unsafe {
            ...
            let node_ptr: *mut Node<usize, V> = self.entry;
...
impl<'a, V: 'a> VacantEntry<'a, V> {
    ...
    pub fn insert(self, value: V) -> &'a mut V {
        ...
        let ret = self.map.map.get_or_insert(key, node);
