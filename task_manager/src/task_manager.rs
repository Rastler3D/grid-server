...
impl TaskManager{
    ...
    pub async fn start(&mut self) -> Result<(), anyhow::Error>{
        loop {
            ...
            match result {
                ...
                Break(Ok(job_result)) => {
                    match &job_result {
                        JobResult::Completed{ distance, ..} => {
                            info!("Job completed. Result: {distance}");
                        }
        ...
        if let Some(ref mut job) = self.current_job{
            match &mut job.state {
                JobState::NotStarted => {
                    ...
                    let taxies = job_data.taxies.len();
                    let passengers = job_data.passengers.len();
                    let task_splitter = TaskSplitter::new(taxies, passengers);
                    let Ok(args) = bincode::serde::encode_to_vec(job_data.clone(),config::standard()) else {
                        return Break(Ok(JobResult::Error{ error_message: "Failed to serialize job arguments".to_string(), stats: None }))
                    };
                    let data = JOB_DATA.iter().map(|&(platform,code)| (platform,code.to_vec())).collect();
                    let worker_job_data = WorkerJobData { args, data };
                    ...
                    job.state = JobState::Started {
                        ...
                        parts: vec![MultilevelVecMap::with_capacity(passengers, (passengers % 64) + 1); taxies],
                ...
                JobState::Started { tasks, parts, phase, stats } => {
                    match phase {
                        JobPhase::Splitting { ref mut splitter } => {
                            ...
                            match splitter.next_chunk::<128>() {
                                Ok(tasks_batch) => {
                                    let mut workers_tasks = Vec::with_capacity(1024);
                                    for (id,task) in tasks_batch {
                                        tasks.insert(id);
                                        let Ok(data) = bincode::serde::encode_to_vec(task, config::standard()) else {
                                            return Break(Ok(JobResult::Error{ error_message: "Failed to serialize task data".to_string(), stats: stats.take() }))
                                        };
                        ...
                        JobPhase::WaitingTasksCompletions => {
                            ...
                            if tasks.is_empty() {
                                let taxies = job.data.taxies.len();
                                let passengers = job.data.taxies.len();
                                let parts = take(parts);

                                *phase = JobPhase::Reducing {
                                    reducer: TaskReducer::new(taxies, passengers, parts),
                                    minimal: Distance::MAX
                                };
                                info!(%job.id, "Job transited to phase 'Reducing'");
                            }
                        }
                        JobPhase::Reducing { reducer, minimal } => {
                            match reducer.next_chunk::<128>() {
                                Ok(distances) => {
                                ...
                                Err(left) => {
                                    ...
                                    return Break(Ok(JobResult::Completed{ distance: *minimal, stats: stats.take() } ))
                                }
                            }
                        }
                    }
    ...
    pub async fn process_scheduler(&mut self, response: SchedulerEvent) -> ControlFlow<Result<JobResult, anyhow::Error>>{
        match response {
            SchedulerEvent::Message(SchedulerResponse::JobComplete(left, job_stats)) => {
                ...
                if let Some(current_job) = &mut self.current_job{
                    if let JobState::Started { parts, tasks , stats, ..  } = &mut current_job.state{
                        *stats = Some(job_stats);
                        for result in left {
                            process_result(parts,tasks,result, *stats)?
                        }
                    }
                }
            },
            ...
            SchedulerEvent::Message(SchedulerResponse::TaskResults(results)) =>{
                info!(results = %results.len(), "Scheduler sent task results");
                if let Some(job) = &mut self.current_job{
                    if let JobState::Started{ parts, tasks , stats, .. } = &mut job.state{
